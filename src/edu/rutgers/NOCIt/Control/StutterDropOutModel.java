...
public class StutterDropOutModel implements SolverModel
{
	final int a_idx = 0;
	final int b_idx = 1;
	...
	public boolean getBounds( double[] x_l, double[] x_u, double[] g_l, double[] g_u )
	{
		x_l[ a_idx ] = 0.0;
		x_u[ a_idx ] = 1.0;
		x_l[ b_idx ] = Double.NEGATIVE_INFINITY;
		x_u[ b_idx ] = 0.0;
...
//		}
	...
	{
		double sum = 0.d;
//                System.out.println("DO obj x="+Arrays.toString(x));
		for( int it = 0; it < vecY.size(); ++it )
		{
//			double p_i = 1.d - ( 1.d / (1.d + Math.exp( -x[ a_idx ] * (vecXs.get( it ) - x[ b_idx ]) )) );
			double p_i = x[ a_idx ] * Math.exp(x[ b_idx ] * vecX.get( it ));
...
//			sum_partial_b += (2 * y_i - 1) / (y_i + e_axb * (1 - y_i)) * e_axb / (1 + e_axb) / a;
			...
			double a = x[ a_idx ];
			double b = x[ b_idx ];
			double x_i = vecX.get( it );	
			double y_i = vecY.get( it );
			double p_i = x[ a_idx ] * Math.exp(x[ b_idx ] * vecX.get( it ));
			
			sum_partial_a += (1 - 2 * y_i) / (p_i * y_i + (1 - p_i) * (1 - y_i)) * Math.exp(b * x_i);
			sum_partial_b += (1 - 2 * y_i) / (p_i * y_i + (1 - p_i) * (1 - y_i)) * a * x_i * Math.exp(b * x_i);
		}

		grad_f[ a_idx ] = sum_partial_a;
		grad_f[ b_idx ] = sum_partial_b;
