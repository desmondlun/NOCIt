 ...
 * @author jkelley
 */
public class AnalyticalThresholdsCEESItController implements Initializable {
    @FXML
    private TableView<ObservableList<?>> table;
    private Stage stage;
    private DrillDownCEESItController ceesitController;
    ...
    public void setCEESItStage(Stage stage, DrillDownCEESItController controller) {
        this.stage = stage;
        this.ceesitController = controller;    
    }
    ...
    private void saveThresholds(ActionEvent event) {
        for (ObservableList<?> row : this.table.getItems()) {
            ...
            for (Locus locus : this.ceesitController.currentThresholdData.keySet()) {
                if (locus.getName().equals(locusName)) {
                    this.ceesitController.currentThresholdData.put(locus, Integer.parseInt(value));
                    break;
                }
            }
        }
    ...
    private void writeAnalyticalThresholdsToFile() {
    	Kit kit = ceesitController.getCurrentCalibration().getKit();
		...
		try {
	        ...
	        for (Locus locus : keys) {
	        	String line = "";
				line += locus.toString() + "\t";
				line += Integer.toString(this.ceesitController.currentThresholdData.get(locus)) + "\t";
